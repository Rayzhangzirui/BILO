...
class BaseProblem(ABC):
    ...
    def validate(self, nn):
        ...
        for vname in nn.trainable_param:
            err = torch.abs(nn.all_params_dict[vname] - self.all_params_dict[vname])
    ...
    def plot_prediction(self, savedir=None):
        ...
        # coord = chr(120 + i) # chr(120) = 'x', chr(121) = 'y', chr(122) = 'z'
        # Plot each dimension
        for i in range(d):
            coord = f'u_{i}'
            ax.plot(x_dat_test, upred[:, i], label=f'pred {coord}')
            ax.plot(x_dat_test, u_test[:, i], label=f'test {coord}', linestyle='--', color='black')
            ax.scatter(x_dat_train, u_dat_train[:, i], label=f'train {coord}', marker='.')
            if 'uinf_dat_test' in self.dataset:
                ax.plot(x_dat_test, self.dataset['uinf_dat_test'][:, i], label=f'inf {coord}', linestyle=':')

            ax.legend()

            if savedir is not None:
                fpath = os.path.join(savedir, f'fig_pred_{coord}.png')
                fig.savefig(fpath, dpi=300, bbox_inches='tight')
                print(f'fig saved to {fpath}')
