...
class WelfordEstimator:
    def __init__(self, burnin: int = 100):
        """
...
class BayesianProblem(BaseProblem):
    ...
    def collect_samples(self, nn):
        ...
        for pname in nn.trainable_param:
            # if scalar parameter
            if isinstance(nn.pde_params_dict[pname], torch.nn.Parameter):
                self.hist[pname].append(nn.pde_params_dict[pname].item())
                self.estimator.update(nn.pde_params_dict[pname], pname)
            # if function parameter
            elif isinstance(nn.pde_params_dict[pname], torch.nn.Module):
                fpred = nn.pde_params_dict[pname](self.dataset['X_dat_test'])
                self.estimator.update(fpred, f'{pname}_pred')
            else:
                raise ValueError(f'unknown type of parameter {pname}')
        
        # collect solution
        u_pred_test = nn(self.dataset['X_dat_test'], nn.pde_params_dict)
        self.estimator.update(u_pred_test, 'u_pred')
    ...
    def collect_solution_MAP(self, net):
        ...
        for pname in net.trainable_param:
            if isinstance(net.pde_params_dict[pname], torch.nn.Parameter):
                # for scalar parameter
                self.MAP_sample[pname] = (net.pde_params_dict[pname].item())
            elif isinstance(net.pde_params_dict[pname], torch.nn.Module):
                # for function
                fpred = net.pde_params_dict[pname](self.dataset['X_dat_test']).squeeze().cpu().numpy()
    ...
    def make_prediction(self, net):
        ...
        for pname in net.trainable_param:
            if isinstance(net.pde_params_dict[pname], torch.nn.Module):
