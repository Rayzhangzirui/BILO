...
class DenseNet(nn.Module):
    def __init__(self, depth=4, width=8, input_dim=1, output_dim=1, 
        ...
        # rbf and fourier cannot be used together
        assert not (self.rbf and self.fourier), 'rbf and fourier cannot be used together'
        ...
        for k, v in all_params_dict.items():
            if isinstance(v, torch.Tensor):
                ...
                tmp[k] = nn.Parameter(torch.tensor([[v]]))
        ...
        self.param_net = []
        self.param_pde = list(self.all_params_dict.values())
    ...
    def setup_modified_mlp(self):
        ...
        if self.fourier:
            ...
            self.input_layer_u = nn.Linear(width, width)
            self.input_layer_v = nn.Linear(width, width)
        elif self.rbf:
            self.rbf_layer = RBF(input_dim, width, self.rff_trainable)
            self.input_layer = nn.Linear(width, width)
            self.input_layer_u = nn.Linear(width, width)
            self.input_layer_v = nn.Linear(width, width)
        else:
            self.input_layer = nn.Linear(input_dim, width)
            self.input_layer_u = nn.Linear(input_dim, width)
    ...
    def collect_trainable_param(self):
        ...
        if self.rbf and self.rff_trainable:
            self.param_net += list(self.rbf_layer.parameters())
    ...
    def embedding_modified(self, x, pde_params_dict=None):
        ...
        if self.with_param :
            ...
            self.param_embedding = param_embedding
        else:
            # for vanilla version, no parameter embedding
            # copy pde_params_dict to params_expand
            for name, param in self.pde_params_dict.items():
            ...
            self.param_embedding = 0

        # fourier feature embedding
        if self.fourier:
            x = self.fflayer(x) + self.param_embedding
            ...
            x = self.rbf_layer(x)
            h = self.input_layer(x) + self.param_embedding
            u = self.input_layer_u(x) + self.param_embedding
            v = self.input_layer_v(x) + self.param_embedding
        else:
            h = self.input_layer(x)
            u = self.input_layer_u(x)
            v = self.input_layer_v(x)
    ...
    def forward_modified(self, x, pde_params_dict=None):
        ...
        for i, hidden_layer in enumerate(self.hidden_layers):
            z = hidden_layer(H)
            if self.skip_param:
                Z = self.act_fun(z + self.param_embedding)
            else:
                Z = self.act_fun(z)
            H = (1 - Z) * U + Z * V
