...
class FKOperatorLearning(FKproblem):
    ...
    def setup_network(self, **kwargs):
        ...
        D0 = 1.0
        rho0 = 1.0        
        opnet.pde_param = torch.nn.Parameter(torch.tensor([D0, rho0], dtype=torch.float32).reshape(1,2).to('cuda'))
    ...
    def get_data_finalt(self, net):
        ...
        upred_res = net(net.pde_param, self.X) #(batch=1, Nt, Nx)
    ...
    def get_data_loss_inverse(self, net):
        ...
        if self.arch == 'fno':
            ...
            upred_dat_train = net(net.pde_param, self.dataset['X_dat_train']).squeeze()
            u_dat_train = self.dataset['u_dat_train'].squeeze()
            loss = torch.nn.functional.mse_loss(upred_dat_train, u_dat_train)
        return loss
    ...
    def validate_inverse(self, net):
        ...
        return {'rD': net.pde_param[0,0].item(), 'rRHO': net.pde_param[0,1].item()}
    ...
    def make_prediction_pretrain(self, net):
        ...
        for i in range(3):
            self.dataset[f'U_{i}'] = U_pred[i].cpu().numpy()

        self.dataset['idx_vis'] = idx   
    ...
    def visualize_pretrain(self, savedir=None):
        ...
        for i in range(3):
            # Create a figure with 3 subplots, adjust the figure size as needed
            fig, ax = plt.subplots(1, 3, figsize=(12, 4))
            Upred = self.dataset[f'U_{i}']
            Ugt = self.dataset['U'][idx[i]]
            # Compute error
            err =  Upred - Ugt
            ...
            fig.colorbar(im_err, ax=ax[2], fraction=0.046, pad=0.04)
    ...
    def make_prediction_inverse(self, net):
        ...
        if self.arch == 'fno':
            ...
            self.dataset['upred_dat'] =       net(net.pde_param, x_dat)
            self.dataset['upred_res'] =       net(net.pde_param, x_res)
            self.dataset['upred_dat_train'] = net(net.pde_param, x_dat_train)
            self.dataset['upred_res_train'] = net(net.pde_param, x_res_train)
